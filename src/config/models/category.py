...
class Category(Base):
    ...
    id_category_iiko: Mapped[Optional[str]] = mapped_column(String(255), default="", nullable=True)
    ...
    des: Mapped[Optional[str]] = mapped_column(String(500), default="", nullable=True)
    show: Mapped[bool] = mapped_column(Boolean, default=True)

    photo: Mapped[Optional[str]] = mapped_column(String(255), default="", nullable=True)

    id_rest_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("restoran_restaurant.id"), nullable=False)
